...
class ImagemController extends Controller
{
    // ✅ RESUMO FINAL
    // Função                                       Local
    // Verifica e limpa imagens órfãs               php artisan syrios:limpar-imagens
    // Visualiza e confirma pela interface master  /master/imagens
    // Remoção segura com confirmação               Sim
    // Proteção entre escolas                       Baseada em prefixo {school_id}
    // Registro de alterações                       Ação feita apenas por master
    public function index()
    {
        ...
        foreach ($arquivos as $path) {
            ...
            if (preg_match('/^(\d+)_(\d+)\.png$/', $nome, $m)) {
                ...
                $alunoExiste ? $validas[] = $nome : $orfas[] = $nome;
            } else {
                $orfas[] = $nome;
            }
        }

        return view('master.imagens.index', compact('validas', 'orfas'));
    }

    public function limpar(Request $request)
    {
        $orfas = $request->input('arquivos', []);
        foreach ($orfas as $img) {
            Storage::delete('public/img-user/' . $img);
        }

        return back()->with('success', '🧹 Imagens órfãs removidas com sucesso!');
    }
}
*/